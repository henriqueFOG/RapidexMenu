...
import styles from "./ApprovedLanding.module.css";

type IconName = "store" | "clipboard" | "rocket" | "dashboard" | "shield" | "link" | "chart" | "clock" | "smile" | "phone" | "report" | "check";
...
function Icon({ name, size = 24 }: { name: IconName; size?: number }) {
  ...
  if (name === "phone") return <svg {...common}><rect x="7" y="2" width="10" height="20" rx="2"/><path d="M10 5h4M11 19h2"/></svg>;
...
const benefits: Array<[IconName, string, string]> = [
  ["chart" as IconName, "Mais oportunidades", "para vender no canal próprio"],
  ["clipboard" as IconName, "Mais praticidade", "para organizar a operação"],
  ["clock" as IconName, "Atendimento mais", "rápido e eficiente"],
  ["smile" as IconName, "Clientes mais", "bem atendidos"],
];
...
export default function ApprovedLanding() {
  const [menuOpen, setMenuOpen] = useState(false);
  const closeMenu = () => setMenuOpen(false);

  return <main className={`${styles.landing} rapidex-approved-landing`}>
    ...
    <section className={styles.restaurantSection} id="para-restaurantes">
      ...
      <div className={styles.dashboardMock} aria-label="Demonstração do painel RapidexMenu">
        <aside>
          <Logo />
          {[["▦","Resumo"],["▤","Pedidos"],["▣","Cardápio"],["♙","Clientes"],["◇","Cupons"],["▥","Relatórios"],["⚙","Configurações"]].map(([i,t],idx)=><span key={t} className={idx===0?styles.dashActive:""}><i>{i}</i>{t}</span>)}
        </aside>
        <div className={styles.dashboardBody}>
          ...
          <div className={styles.dashboardGrid}>
            <article className={styles.chartCard}>
              ...
              <svg viewBox="0 0 420 160" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="area" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#ff650b" stopOpacity=".23"/><stop offset="1" stopColor="#ff650b" stopOpacity="0"/></linearGradient></defs><path d="M0 128 C35 105 55 82 80 100 S120 135 150 92 S198 65 226 95 S270 125 303 82 S350 96 420 34 L420 160 L0 160Z" fill="url(#area)"/><path d="M0 128 C35 105 55 82 80 100 S120 135 150 92 S198 65 226 95 S270 125 303 82 S350 96 420 34" fill="none" stroke="#ff650b" strokeWidth="3"/><g fill="#ff650b">{[[0,128],[80,100],[150,92],[226,95],[303,82],[420,34]].map(([x,y])=><circle key={x} cx={x} cy={y} r="4"/>)}</g></svg>
            ...
            <article className={styles.ordersCard}>
              ...
              {[ ["#1048","R$ 59,90","Novo"],["#1047","R$ 34,90","Preparando"],["#1046","R$ 89,90","Saiu para entrega"],["#1045","R$ 29,90","Entregue"] ].map((r,i)=><div key={r[0]}><b>{r[0]}</b><span>{r[1]}</span><em className={styles[`status${i}` as keyof typeof styles]}>{r[2]}</em></div>)}
            </article>
          </div>
        </div>
      </div>
    ...
    <section className={styles.how} id="como-funciona">
      ...
      <div className={styles.steps}>
        ...
        ].map(([icon,title,text],i)=><article key={title}><div className={styles.stepIcon}><Icon name={icon as IconName} size={34}/></div><span>{i+1}</span><h3>{title}</h3><p>{text}</p>{i<3&&<i className={styles.arrow}>→</i>}</article>)}
      </div>
    </section>
    ...
    <section className={styles.plans} id="planos">
      ...
      <div className={styles.planGrid}>
        {plans.map(plan=><article key={plan.name} className={plan.featured?styles.featuredPlan:""}>
          {plan.featured&&<div className={styles.planTop}><b>{plan.name}</b><span>Mais escolhido</span></div>}
          {!plan.featured&&<b className={styles.planName}>{plan.name}</b>}
