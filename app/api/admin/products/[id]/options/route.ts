    ...
    const options = group.options.map((rawOption, optionIndex): NormalizedOption => {
      ...
      if (kind === "variant") {
        ...
        return {
          ...
          priceDeltaCents: 0,
          costDeltaCents: 0,
          finalPriceCents: integerBetween(option.finalPriceCents, 1, 1_000_000, "Preço da variação"),
          finalCostCents: integerBetween(option.finalCostCents ?? 0, 0, 1_000_000, "Custo da variação"),
          available,
          stockControlEnabled,
          stockQuantity,
        };
