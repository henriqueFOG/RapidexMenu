-- Product variants are modeled as a first-class kind of option group. This keeps
-- the existing modifier model compatible while giving sizes/volumes their own
-- price, cost, availability and stock invariant.
...
CREATE INDEX IF NOT EXISTS product_variant_stock_idx
  ON product_options (group_id, available, stock_control_enabled, stock_quantity);

-- statement-breakpoint

-- Snapshot whether an order choice was a structural variant or a normal modifier.
-- Historical price/name snapshots already live in order_item_options.
...
-- Variant inventory is reserved atomically while the order transaction inserts
-- its option snapshot. The option row lock serializes concurrent purchases.
...
-- Cancellation returns variant stock once. The transition guard makes retries of
-- the same canceled state harmless. Base product stock is disabled whenever a
-- product has variants, so this trigger composes safely with product-level stock.
